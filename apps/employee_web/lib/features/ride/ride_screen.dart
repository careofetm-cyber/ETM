...
class _RideScreenState extends ConsumerState<RideScreen> {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            FilledButton.icon(onPressed: _loadRideInfo, icon: const Icon(Icons.refresh), label: const Text('Retry')),
          ],
        ),
      ),
    );
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('You don\'t have an assigned cab right now.', textAlign: TextAlign.center,
              style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant)),
            const SizedBox(height: 24),
            FilledButton.icon(onPressed: _loadRideInfo, icon: const Icon(Icons.refresh), label: const Text('Refresh')),
          ],
        ),
      ),
    );
    ...
    return SingleChildScrollView(
      ...
      child: isWide
          ? Row(
              ...
              children: [
                Expanded(child: _buildTripInfoCard(routeName, vehiclePlate, driverName, timePart, cs)),
                const SizedBox(width: 24),
                Expanded(child: Column(
                  children: [
                    _buildOtpDisplay(otp, isExpired, remainingMin, cs),
                    const SizedBox(height: 16),
                    _buildSOSButton(),
                    if (_passengers.isNotEmpty) ...[
                      const SizedBox(height: 16),
                      _buildPassengersList(cs),
                    ],
                  ],
                )),
              ],
          ...
          : Column(
              children: [
                _buildTripInfoCard(routeName, vehiclePlate, driverName, timePart, cs),
                const SizedBox(height: 16),
                _buildOtpDisplay(otp, isExpired, remainingMin, cs),
                const SizedBox(height: 16),
                _buildSOSButton(),
  ...
  Widget _buildTripInfoCard(String routeName, String vehiclePlate, String driverName, String timePart, ColorScheme cs) {
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(20),
        child: Column(
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      if (driverName.isNotEmpty) Text('Driver: $driverName', style: Theme.of(context).textTheme.bodySmall?.copyWith(color: Colors.grey)),
                    ],
                  ),
                ),
              ],
  ...
  Widget _buildOtpDisplay(String otp, bool isExpired, int remainingMin, ColorScheme cs) {
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: isExpired
            ...
            : LinearGradient(colors: [cs.primaryContainer.withOpacity(0.5), cs.primaryContainer.withOpacity(0.2)]),
        borderRadius: BorderRadius.circular(20),
        border: Border.all(color: isExpired ? Colors.red.withOpacity(0.3) : cs.primary.withOpacity(0.15)),
      ),
      child: Column(
        children: [
          Container(
            ...
            child: Icon(isExpired ? Icons.timer_off : Icons.pin, size: 32, color: isExpired ? Colors.red : cs.primary),
          ),
          const SizedBox(height: 16),
          Text(
            isExpired ? 'OTP Expired' : 'Show this OTP to your driver',
            style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: isExpired ? Colors.red : cs.onSurfaceVariant),
          ),
          const SizedBox(height: 16),
          Text(
            otp,
          ...
          if (isExpired)
            ...
            Container(
              ...
              child: Text('Valid for $remainingMin min', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: cs.primary)),
            ),
        ],
      ),
    );
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(16),
            boxShadow: [BoxShadow(color: const Color(0xFFDC2626).withOpacity(0.3), blurRadius: 8, offset: const Offset(0, 4))],
          ),
          child: const Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              ...
              SizedBox(width: 10),
              Text('SOS - Emergency', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: Colors.white)),
            ],
          ),
        ),
      ),
  ...
  Widget _buildPassengersList(ColorScheme cs) {
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(20),
        child: Column(
              ...
              tiles: _passengers.map((p) {
                ...
                return ListTile(
                  leading: CircleAvatar(
                    ...
                    child: Icon(isDropped ? Icons.check_circle : isBoarded ? Icons.person : Icons.person_outline, color: statusColor, size: 20),
                  ),
          ...
          Container(
            ...
            child: Column(
              children: [
                Container(
                  padding: const EdgeInsets.all(16),
                  decoration: BoxDecoration(color: const Color(0xFF059669).withOpacity(0.15), shape: BoxShape.circle),
                  child: Icon(isInProgress ? Icons.directions_bus : Icons.check_circle, size: 48, color: const Color(0xFF059669)),
                ),
                const SizedBox(height: 16),
                Text(
                ...
                const SizedBox(height: 8),
                Text(
                  isInProgress ? 'Your driver is on the way. Track your trip live.' : 'Your OTP has been verified. Trip is in progress.',

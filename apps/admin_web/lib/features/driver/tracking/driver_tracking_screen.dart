...
class DriverTrackingScreen extends ConsumerStatefulWidget {
  final String? tripId;
  const DriverTrackingScreen({super.key, this.tripId});
...
class _DriverTrackingScreenState extends ConsumerState<DriverTrackingScreen> {
  ...
  Future<void> _loadData() async {
    try {
      ...
      final tripId = widget.tripId ?? _activeTripId;
      if (tripId == null) {
        final activeResp = await dio.get('/trips', queryParameters: {'status': 'in_progress'});
        final trips = activeResp.data['data'] ?? [];
        if (trips.isEmpty) {
          if (mounted) setState(() { _isLoading = false; });
          return;
        }
        _activeTripId = trips.first['id']?.toString();
        if (_activeTripId == null) {
          if (mounted) setState(() { _isLoading = false; });
          return;
        }
      }
      final effectiveTripId = widget.tripId ?? _activeTripId!;
      final locResp = await dio.get('/trips/$effectiveTripId/location');
      final passResp = await dio.get('/trips/$effectiveTripId/passengers');
      if (mounted) {
        setState(() {
  ...
  Future<void> _sendSos() async {
    ...
    if (confirmed == true && mounted) {
      try {
        final dio = ref.read(dioProvider);
        await dio.post('/trips/${widget.tripId ?? _activeTripId}/sos');
        if (mounted) {
    ...
    return Padding(
      ...
      child: Column(
        children: [
          Row(
            children: [
              Text('Live Tracking', style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold)),
              const Spacer(),
              Container(
          ...
          Expanded(
            child: Row(
              children: [
                Expanded(
                  flex: 3,
                  child: _buildMapArea(driverLat, driverLng, stops, cs),
                ),
                const SizedBox(width: 16),
                Expanded(
                  flex: 2,
                  child: _buildSidePanel(passengerCount, boardedCount, droppedCount, cs),
                ),
              ],
            ),
          ),
  ...
  Widget _buildMapArea(dynamic driverLat, dynamic driverLng, List stops, ColorScheme cs) {
    return Container(
  ...
  Widget _buildSidePanel(int passengerCount, int boardedCount, int droppedCount, ColorScheme cs) {
    return Column(
      children: [
        Row(
          children: [
...
class _RoutePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    path.quadraticBezierTo(size.width * 0.3, size.height * 0.6, size.width * 0.5, size.height * 0.45);
    path.quadraticBezierTo(size.width * 0.7, size.height * 0.3, size.width * 0.9, size.height * 0.7);

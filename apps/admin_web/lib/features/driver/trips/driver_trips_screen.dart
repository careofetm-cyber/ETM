...
class _DriverTripsScreenState extends ConsumerState<DriverTripsScreen> with SingleTickerProviderStateMixin {
                ...
                children: [
                  ...
                  Expanded(
                    child: TabBarView(
                      ...
                      children: [
                        useTable
                            ? _buildTableView(_todayTrips, showAction: true, padding: padding)
                            : _buildCardView(_todayTrips, showAction: true, padding: padding),
                        useTable
                            ? _buildTableView(_upcomingTrips, showAction: false, padding: padding)
                            : _buildCardView(_upcomingTrips, showAction: false, padding: padding),
                        useTable
                            ? _buildTableView(_completedTrips, showAction: false, padding: padding, isCompleted: true)
                            : _buildCompletedCardView(_completedTrips, padding: padding),
                      ],
                    ),
                  ),
                ],
  ...
  Widget _buildTableView(List<dynamic> trips, {required bool showAction, double padding = 16, bool isCompleted = false}) {
    final cs = Theme.of(context).colorScheme;
    if (trips.isEmpty) {
      return Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(isCompleted ? Icons.check_circle_outlined : Icons.route_outlined,
                size: 56, color: cs.onSurfaceVariant.withOpacity(0.4)),
            const SizedBox(height: 12),
            Text(isCompleted ? 'No completed trips' : 'No trips found',
        ...
        child: SingleChildScrollView(
          child: DataTable(
            columns: [
              ...
              const DataColumn(label: Text('Passengers')),
              if (!isCompleted) const DataColumn(label: Text('Status')),
            ...
            rows: trips.map((trip) {
              ...
              return DataRow(cells: [
                      ...
                      Column(
                        ...
                        children: [
                          ...
                          if (vehiclePlate.isNotEmpty)
                            Text(vehiclePlate, style: TextStyle(fontSize: 12, color: cs.onSurfaceVariant)),
                        ],
                      ),
                    ],
                  ),
                    ...
                    child: Row(
                      ...
                      children: [
                        Icon(isPickup ? Icons.arrow_upward : Icons.arrow_downward,
                            size: 12, color: isPickup ? cs.primary : cs.tertiary),
                ...
                DataCell(Text(timePart, style: const TextStyle(fontWeight: FontWeight.w600))),
                DataCell(Text('$boardedCount/$passengerCount')),
                if (!isCompleted)
                  DataCell(
                    Container(
                      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                    ...
                    children: [
                      ...
                      if (status == 'inProgress')
                        IconButton(
                          ...
                          onPressed: () => context.push('/driver/tracking/${trip['id']}'),
                          tooltip: 'Track',
                        ),
                    ],
                  ),
  ...
  Widget _buildCardView(List<dynamic> trips, {required bool showAction, double padding = 16}) {
    final cs = Theme.of(context).colorScheme;
    if (trips.isEmpty) {
      return Center(
        child: Column(
                      ...
                      Container(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Text('$boardedCount/$passengerCount pax',
  ...
  Widget _buildCompletedCardView(List<dynamic> trips, {double padding = 16}) {
    final cs = Theme.of(context).colorScheme;
    if (trips.isEmpty) {
      return Center(
        child: Column(

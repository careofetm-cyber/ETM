              ...
              data: (data) => Wrap(
                ...
                children: [
                  StatCard(
                    ...
                    value: '${data['totalCompanies'] ?? data['total_companies'] ?? 0}',
                    icon: Icons.business,
                    color: AppColors.primary,
                  ),
                  StatCard(
                    title: 'Active Companies',
                    value: '${data['activeCompanies'] ?? data['active_companies'] ?? 0}',
                    icon: Icons.check_circle,
                    color: AppColors.success,
                  ),
                  StatCard(
                    title: 'Total Trips This Month',
                    value: '${data['totalTripsThisMonth'] ?? data['total_trips_this_month'] ?? 0}',
                    icon: Icons.trip_origin,
                    color: AppColors.info,
                  ),
                  StatCard(
                    title: 'Total Revenue',
                    value: '\$${((data['totalRevenue'] ?? data['total_revenue_this_month'] ?? 0) as num?)?.toDouble()?.toStringAsFixed(2) ?? '0.00'}',
                    icon: Icons.attach_money,
                    color: AppColors.accent,
                  ),
                  StatCard(
                    title: 'Total Drivers',
                    value: '${data['totalDrivers'] ?? data['total_drivers'] ?? 0}',
                    icon: Icons.person,
                    color: AppColors.secondary,
                  ),
                  StatCard(
                    title: 'Total Vehicles',
                    value: '${data['totalVehicles'] ?? data['total_vehicles'] ?? 0}',
                    icon: Icons.directions_bus,
                    color: AppColors.warning,
                  ),
                  StatCard(
                    title: 'Total Employees',
                    value: '${data['totalEmployees'] ?? data['total_employees'] ?? 0}',
                    icon: Icons.people,
                    color: AppColors.info,
                  ),
                ],

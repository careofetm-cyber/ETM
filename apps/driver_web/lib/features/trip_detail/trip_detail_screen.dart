...
class _TripDetailScreenState extends ConsumerState<TripDetailScreen> {
      ...
      body: _isLoading
          ...
          : _error != null
              ...
              : RefreshIndicator(
                  onRefresh: _loadTrip,
                  child: useWideLayout
                      ? _buildWideLayout(status, routeName, vehiclePlate, timePart, distance, statusColors, cs, padding)
                      : _buildNarrowLayout(status, routeName, vehiclePlate, timePart, distance, statusColors, cs, padding),
                ),
    );
  }

  Widget _buildWideLayout(String status, String routeName, String vehiclePlate, String timePart,
      dynamic distance, Map<String, Color?> statusColors, ColorScheme cs, double padding) {
    return Row(
      ...
      children: [
        Expanded(
          ...
          child: ListView(
            ...
            children: [
              _buildTripInfoCard(routeName, vehiclePlate, timePart, distance, statusColors, status, cs),
              if (status == 'scheduled') ...[
                const SizedBox(height: 16),
        ...
        Expanded(
          flex: 4,
          child: _buildPassengerTable(status, cs, padding),
        ),
      ],
    );
  }

  Widget _buildNarrowLayout(String status, String routeName, String vehiclePlate, String timePart,
      dynamic distance, Map<String, Color?> statusColors, ColorScheme cs, double padding) {
    return ListView(
      padding: EdgeInsets.all(padding),
      children: [
        ...
        _buildPassengerList(status, cs),
      ],
    );
  }

  Widget _buildTripInfoCard(String routeName, String vehiclePlate, String timePart, dynamic distance,
      Map<String, Color?> statusColors, String status, ColorScheme cs) {
    return Card(
      child: Padding(
  ...
  Widget _buildPassengerTable(String status, ColorScheme cs, double padding) {
    return Padding(
      padding: EdgeInsets.all(padding),
      child: Card(
        child: Column(
              ...
              child: _passengers.isEmpty
                  ? Center(
                      child: Column(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          Icon(Icons.people_outline, size: 40, color: cs.onSurfaceVariant.withOpacity(0.4)),
                      ...
                      child: DataTable(
                        ...
                        rows: _passengers.map((p) {
                          ...
                          return DataRow(cells: [
                            DataCell(Text(name.isNotEmpty ? name : 'Unknown',
                                style: const TextStyle(fontWeight: FontWeight.w600))),
                            DataCell(Text(employeeId, style: const TextStyle(fontSize: 13))),
                            DataCell(
                              Container(
                            ...
                            DataCell(Text(p['stopName'] ?? '-',
                                style: TextStyle(color: cs.onSurfaceVariant, fontSize: 13))),
                            DataCell(
                              (status == 'scheduled' && !isBoarded && !isDropped && !isNcns)
                                  ? Row(
                                      mainAxisSize: MainAxisSize.min,
                                      children: [
  ...
  Widget _buildPassengerList(String status, ColorScheme cs) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(
